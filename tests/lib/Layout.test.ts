...
test(`${keyToType.name}: Returns correct types`, () => {
	...
	const types = Object.keys(exampleData).map((key) =>
		keyToType(League.FRC, key, exampleData),
	);
...
test(`${keyToType.name}: Chooses FrcDrivetrain or FtcDrivetrain correctly`, () => {
	expect(
		keyToType(League.FRC, "FrcDrivetrain", { FrcDrivetrain: "Tank" }),
	).toBe(FrcDrivetrain);
	expect(
		keyToType(League.FTC, "FtcDrivetrain", { FtcDrivetrain: "Tank" }),
	).toBe(FtcDrivetrain);
});
...
test(`${FormElement.name}.${FormElement.fromProps.name}: From props`, () => {
	...
	const element = FormElement.fromProps(
		League.FRC,
