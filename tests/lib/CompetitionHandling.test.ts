...
test(`${generateSchedule.name}: Generates a schedule with the correct number of matches`, () => {
  ...
  const schedule = generateSchedule(scouters, subjectiveScouters, matchCount, robotsPerMatch);
...
test(`${generateSchedule.name}: Generates a schedule with the correct number of scouters per match`, () => {
  const scouters = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p"];
  const subjectiveScouters = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p"];
...
test(`${generateSchedule.name}: Rotates scouters `, () => {
  ...
  for (let i = 1; i < matchCount; i++) {
    expect(schedule[i].assignedScouters).not.toEqual(schedule[i - 1].assignedScouters);
  }
});
