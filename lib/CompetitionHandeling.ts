...
import { Competition, Match, Team, Report, AllianceColor, FormData } from "./Types";
...
export async function AssignScoutersToCompetitionMatches(teamId: string, competitionId: string, shuffle: boolean=false) {
...
export async function AssignScoutersToMatch(matchId: string, scouterArray: string[], shuffleScouters: boolean = false): Promise<void> {
    const db = await GetDatabase();
