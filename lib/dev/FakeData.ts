...
async function fetchNames(): Promise<[string[], string[]]> {
    if(cachedFirstNames.length === 0) {
        cachedFirstNames = cachedFirstNames.concat((await (await fetch(firstNameFemaleURL)).json()).data)
        cachedFirstNames = cachedFirstNames.concat((await (await fetch(firstNameMaleURL)).json()).data)
    } if(cachedLastNames.length === 0) {
        cachedLastNames = (await (await fetch("https://www.randomlists.com/data/names-surnames.json")).json()).data;
    }
...
export async function fakeUser(teamId: string | undefined): Promise<User> {
    ...
    false, await GenerateSlug(Collections.Users, name), teamId ? [teamId]: [], [], '', 10);
    return await db.addObject<User>(Collections.Users, user);
}

export async function fillTeamWithFakeUsers(n: number, teamId: string | undefined): Promise<Team> {
    ...
    const team = await db.findObjectById<Team>(Collections.Teams, new ObjectId(teamId));
