...
const options: MongoClientOptions = {maxPoolSize: 1};
...
export class MongoDBInterface {
    ...
    async updateObjectById<Type>(collection: Collections, id: ObjectId, newValues: object): Promise<Type> {
        var query = {"_id": id};
        var updated = {$set: newValues};
        return await this?.db?.collection(collection).updateOne(query, updated) as Type;
    }

    async findObjectById<Type>(collection: Collections, id: ObjectId): Promise<Type> {
        var query = {"_id": id};
        return await this?.db?.collection(collection).findOne(query) as Type;
    }

    async findObject<Type>(collection: Collections, query: object): Promise<Type> {
        return await this?.db?.collection(collection).findOne(query) as Type;
    }

    async findObjects<Type>(collection: Collections, query: object): Promise<Type[]> {
        return await this?.db?.collection(collection).find(query).toArray() as Type[];
    }
}
