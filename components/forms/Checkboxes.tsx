...
export default function Checkbox(props: CheckboxProps) {
  ...
  return (
    <label className="w-full label cursor-pointer flex flex-row justify-end space-x-8">
      <span className="label-text text-xl font-semibold ">
        {props.label} + {props.dataKey}
      </span>
      ...
      <input
        ...
        checked={checked}
        className="w-10 h-10 checkbox checkbox-primary"
      />
    </label>
  );
}
...
export function IntakeType(props: RadioProps) {
  return (
    <div className="w-full grid grid-cols-2 grid-rows-3 items-center text-2xl">
      <span>Human Intake: </span>{" "}
      <input
        type="radio"
