...
export default function Summary(props: {
			...
			<code>
				<div>
					{props.dots.map((dot, index) => (
						<div
							key={index}
							className="badge badge-sm badge-primary w-full"
						>
							{dot.label}
						</div>
					))}
	...
	const avgX = NumericalAverage(
		(r) => r.AutoStart?.x ?? 0,
		props.selectedReports,
	);
	const avgA = NumericalAverage(
		(r) => r.AutoStart?.y ?? 0,
		props.selectedReports,
	);
	...
	return (
		<code className=" w-full h-full mx-auto text-sm font-semibold ">
			...
			{matches < 5 ? (
				...
				<>
					This robot has competed in {matches} matches and is very well
					characterized.
				</>
			)}
			<div>
