...
export default function Home(props: ResolvedUrlData) {
  ...
  useEffect(() => {
    const loadMatches = async () => {
      ...
      setQualifyingMatches(
        newMatches.filter((match) => match.type === MatchType.Qualifying),
      );
      setSemiFinalMatches(
        newMatches.filter((match) => match.type === MatchType.Semifinals),
      );
      setFinalMatches(
        newMatches.filter((match) => match.type === MatchType.Finals),
      );
      setLoadingMatches(false);
      setNumberSubmitted(submissionCounter);
      setSubmissionRate(
        Math.round((submissionCounter / Object.keys(newReports).length) * 100) /
          100,
      );
  ...
  function DisplayMatch(props: { match: Match; users: User[] }) {
    ...
    const repElements = reps.map((rep, index) => {
      ...
      return (
        <>
          <Link
            href={`/${team?.slug}/${season?.slug}/${comp?.slug}/${rep._id}`}
            key={index}
          >
            <div
              className={`w-10 h-10 ${submitted ? "bg-gray-500" : rep.color === "Blue" ? "bg-blue-500" : "bg-red-500"} border-2 border-white rounded-lg flex flex-row items-center justify-center`}
            >
              {isItUs ? (
                <BsStarFill className="text-yellow-500 text-2xl"></BsStarFill>
              ) : (
                <></>
              )}
              {isMe ? (
                <AiOutlineUser className="text-white text-2xl"></AiOutlineUser>
              ) : (
                <></>
              )}
              {someoneActive ? (
                <BsCheckCircle className="text-white text-2x1"> </BsCheckCircle>
              ) : (
                <></>
              )}
            </div>
              ...
              {slackUsers.map((user, index) => {
                return (
                  ...
                  >
                    <IoIosNotifications
                      size={50}
                      className="text-4xl inline-block text-white"
                    ></IoIosNotifications>{" "}
                    {user?.name}
                  </li>
                );
          ...
          <div className="w-full flex flex-col items-center ">
            {showKey ? (
              <div className="mb-10">
                ...
                <h1>
                  <AiOutlineUser className="inline-block text-2xl"></AiOutlineUser>{" "}
                  = Match Assigned To You
                </h1>
                <h1>
                  <BsStarFill className="inline-block text-2xl"></BsStarFill> =
                  Your Team
                </h1>
              </div>
            ) : (
            ...
            <div className="tabs ">
              ...
              <a
                className={`tab tab-bordered ${tab === 3 ? "tab-active" : ""}`}
                onClick={() => {
                  setTab(3);
                }}
              >
            ...
            <h1 className="card-title mt-6">
              {name} ({matches.length})
            </h1>
            {loadingMatches ? (
              <div className="flex flex-col items-center">
                <span className="loading loading-spinner loading-lg"></span>
                <p className="animate-pulse mt-6 text-xl">
                  Loading... (this will take awhile)
                </p>
              </div>
            ) : (
              <></>
  ...
  function Settings() {
    ...
    const canAssign = team?.scouters
      ? team?.scouters.length >= 6
        ? true
        : false
      : false;
      ...
      <div className="card w-5/6 bg-base-200 shadow-xl">
        <div className="card-body">
          ...
          <input
            ...
            value={matchNumber}
            onChange={(e) => setMatchNumber(e.target.valueAsNumber)}
          ...
          <input
            type="number"
            placeholder="Red Alliance 1"
            value={redAlliance[0]}
            onChange={(e) => {
              targetRedAlliance(0, e.target.valueAsNumber);
            }}
            className="input input-bordered input-primary w-full max-w-xs"
          />
          <label>Team 2</label>
          <input
            type="number"
            placeholder="Red Alliance 2"
            value={redAlliance[1]}
            onChange={(e) => {
              targetRedAlliance(1, e.target.valueAsNumber);
            }}
            className="input input-bordered input-primary w-full max-w-xs"
          />
          <label>Team 3</label>
          <input
            type="number"
            placeholder="Red Alliance 3"
            value={redAlliance[2]}
            onChange={(e) => {
              targetRedAlliance(2, e.target.valueAsNumber);
            }}
            className="input input-bordered input-primary w-full max-w-xs"
          />
          ...
          <input
            type="number"
            placeholder="Blue Alliance 1"
            value={blueAlliance[0]}
            onChange={(e) => {
              targetBlueAlliance(0, e.target.valueAsNumber);
            }}
            className="input input-bordered input-primary w-full max-w-xs"
          />
          <label>Team 2</label>
          <input
            type="number"
            placeholder="Blue Alliance 2"
            value={blueAlliance[1]}
            onChange={(e) => {
              targetBlueAlliance(1, e.target.valueAsNumber);
            }}
            className="input input-bordered input-primary w-full max-w-xs"
          />
          <label>Team 3</label>
          <input
            type="number"
            placeholder="Blue Alliance 3"
            value={blueAlliance[2]}
            onChange={(e) => {
              targetBlueAlliance(2, e.target.valueAsNumber);
            }}
            className="input input-bordered input-primary w-full max-w-xs"
          />

          <p className="italic">
            Note: Manually Creating a Match Requires Eventual Scouter
            Reassignment
          </p>
      ...
      <div className="w-full h-full flex flex-col items-center justify-center space-y-6">
        <div className="card w-5/6 bg-base-200 shadow-xl">
          <div className="card-body">
            ...
            <Link href={`/${team?.slug}/${season?.slug}/${comp?.slug}/stats`}>
            ...
            <div className="stats bg-base-300 stats-vertical lg:stats-horizontal lg:w-1/2">
              <div className="stat ">
                ...
                <div className="stat-desc">
                  {Object.keys(reports).length - numberSubmitted} Matches
                  Remaining
                </div>
              </div>
              ...
              <div className="stat">
                ...
                <div className="stat-desc">
                  {(Math.round(
                    (missedMatches / Object.keys(reports).length) * 100,
                  ) /
                    100) *
                    100}
                  % of total matches
