...
export default function Scouters(props: {
	team: Team | null;
	...
	const [scouters, setScouters] = useState<
		{ [id: string]: Scouter } | undefined
	>();
	const [shouldRegenerateScouterData, setShouldRegenerateScouterData] =
	...
	>();
	const [lastCountedMatch, setLastCountedMatch] = useState<number>(
	...
	useEffect(() => {
		...
		api.findScouterManagementData(comp?._id!).then((data) => {
					...
					func(comment).then(() =>
						setComments((prev) => [
							...(prev || []).filter((c) => c !== comment),
							{
								...comment,
								text: "",
								flag: getCommentFlag(comment.text, false),
							},
						]),
					);
			...
			function removeSubjectiveComment(comment: Comment) {
				return api.updateSubjectiveReport({
					_id: comment.dbId,
					wholeMatchComment: "",
					robotComments: {},
				} as SubjectiveReport);
			...
			for (const report of data.subjectiveReports) {
				const text = [
					report.wholeMatchComment.length > 0
						? `Whole Match: ${report.wholeMatchComment}`
						: "",
					...Object.entries(report.robotComments).map(
						([key, value]) => value.length > 0 && `Robot ${key}: ${value}`,
					),
				]
					.filter((c) => c)
					.join(" \\ ");
				...
				comments.push({
					...
					remove: () =>
						removeComment(report._id, (c) => removeSubjectiveComment(c)),
				});
			}
			...
			const newScouters = Object.values(scouters).map((scouter) => {
				...
				return {
					...
					missedSubjectiveReports: Object.values(matches)
						.filter(
							(match) =>
								match.number <= lastCountedMatch &&
								match.subjectiveScouter === scouter._id,
						)
						.map((match) => match._id ?? ""),
				};
			});
	...
	return (
		<Container
			...
			hideMenu={false}
			title={`Scouter Management | ${props.competition?.name}`}
		>
			{!isManager ? (
				...
				<div className="flex flex-row max-sm:flex-col justify-center">
					<div className="card w-1/3 max-sm:w-3/4 bg-base-200 m-12">
						<div className="card-body h-screen overflow-y-scroll">
							...
							<p>
								Scouters: {scouters && Object.keys(scouters)?.length}
								<br />
								Matches: {comp && comp?.matches.length}
								<br />
								Reports:{" "}
								{reports ? (
								...
								/{comp && comp?.matches.length * 6} (submitted/total) <br />
								<span
									className="tooltip"
									data-tip="Reports for this match and any before it that are not submitted will be considered missing."
								>
									Last Counted Match:
								</span>
								<input
									type="number"
									value={lastCountedMatch}
									className="input input-bordered input-sm ml-2 w-1/5 sm:w-1/6"
									onChange={(e) => {
										setShouldRegenerateScouterData(true);
										setLastCountedMatch(
											Math.max(
							...
							<ul>
								{scouters &&
									Object.values(scouters)
										?.filter((scouter) => scouter.reports.length > 0)
										.sort(
											(a, b) => b.missedReports.length - a.missedReports.length,
										)
										.map((scouter, index) => (
											<li key={index}>
												<span
												...
												<ul className="text-sm ml-2 mb-1">
													...
													{matches &&
														scouter.missedSubjectiveReports.length > 0 && (
															<li>
																Missed Subjective Reports:{" "}
																{[
																	...new Set(
													...
													{scouter.coveredReports.length > 0 && (
														<li>
													...
													<li>
														Submitted Reports:{" "}
														{scouter.reports.length -
															scouter.missedReports.length}
														/{scouter.reports.length}
													</li>
												</ul>
											</li>
