...
export default function SubjectiveReportForm(props: {
	compId?: string;
	teamNumber?: number;
	compName?: string;
}) {
	...
	function getReportFromForm(): SubjectiveReport {
		return {
			...
			robotComments: Object.fromEntries(
				match?.blueAlliance
					.concat(match.redAlliance)
					.map((team: number, index: number) => [
						match?.blueAlliance.concat(match.redAlliance)[index],
						(document.getElementById(`comment-${team}`) as HTMLTextAreaElement)
							.value ?? "",
					]) ?? [],
			),
			submitter: undefined,
	...
	async function submit(e: FormEvent<HTMLFormElement>) {
		...
		let valid =
			[...(e.target as any)].filter((element: any) => element.value).length > 0;
		if (!valid) {
		...
		api
			.submitSubjectiveReport(getReportFromForm(), matchId as string)
			.then(() => {
				const teamsWithComments = [...(e.target as any)].map(
				...
				Analytics.subjectiveReportSubmitted(
					teamsWithComments,
					props.teamNumber ?? -1,
					props.compName ?? "Unknown Competition",
	...
	useInterval(
		() =>
			api.checkInForSubjectiveReport(matchId as string).catch(console.error),
		5000,
		[router, match?._id],
	);
					...
					<>
						<div className="card w-[80%] bg-base-200">
							<div className="card-body">
								<div className="card-title">Match {match.number}</div>
								<form
								...
								>
									{[
										...
										<div key={index}>
											<div className="label">
												<span
													className={`label-text text-lg ${
														typeof team === "number" &&
														(match.blueAlliance.includes(team)
															? "text-blue-500"
															: "text-red-500")
													}`}
												>
											...
											<textarea
												id={`comment-${team.toString().replace(" ", "")}`}
									...
									<button
										ref={submitButtonRef}
										className={`btn btn-${submitting ? "disabled" : "primary"} mt-6`}
									>
										{submitting ? (
											<Loading
												bg=""
