...
import Checkbox, { DrivetrainType, IntakeType } from "@/components/forms/Checkboxes";
    ...
    return (
      <Container requireAuthentication={false} hideMenu={!hide}>
        <div className="w-screen flex flex-col items-center justify-center space-y-4">
            <div className="card w-2/3 bg-base-200 mt-2">
            ...
            <div className="card w-2/3 bg-base-200 mb-2">
                <div className="card-body w-full flex flex-col justify-center items-center text-center">
                    ...
                    <div className="translate-x-2">
                        ...
                        <DrivetrainType data={pitreport} callback={setCallback}></DrivetrainType>
                    </div>
                    

                    <Checkbox label="Can Climb" dataKey="canClimb" data={pitreport} callback={setCallback}></Checkbox>
                    <Checkbox label="Can Score Amp" dataKey="canScoreAmp" data={pitreport} callback={setCallback}></Checkbox>
                    <Checkbox label="Can Score Speaker" dataKey="canScoreSpeaker" data={pitreport} callback={setCallback}></Checkbox>

                    <div className="w-1/2">
                    <CommentBox data={pitreport} callback={setCallback}></CommentBox>
                    </div>

                    <button className="btn btn-wide btn-primary " onClick={submit}>Submit</button>
...
async function getPitreport(id: string) {
    const db = await gdb;
    return await db.findObjectById<Pitreport>(Collections.Pitreports, new ObjectId(id))
}
