...
export default function PublicEvent(props: {comp: Competition, firstRanking: TheBlueAlliance.SimpleRank[] , oprRanking: TheBlueAlliance.OprRanking, statbotics: Statbotics.TeamEvent[]}) {
                ...
                <div className="card-body">
                    ...
                    <div className="w-full h-full flex flex-row">
                        <div className="w-full h-full">
                            ...
                            <h1 className="italic">Publically available data combined with our statistics engine</h1>
                            <table className="table">
                                ...
                                <tbody>
                                    {first.map((ranking: any, index: number) => <tr key={ranking.team_key}>
                                        ...
                                        <th>{findStatboticsStats(ranking.team_key)?.record.qual.rank}</th>
                                        ...
                                        <td>{findStatboticsStats(ranking.team_key)?.record.qual.winrate.toFixed(2)}</td>
                                        <td>{oprs[ranking.team_key].toFixed(2)}</td>
                                        <td>{findStatboticsStats(ranking.team_key)?.epa.total_points.mean.toFixed(2)}</td>
                                        <td>{findStatboticsStats(ranking.team_key)?.epa.breakdown.teleop_points.mean.toFixed(2)}</td>
                                        <td>{findStatboticsStats(ranking.team_key)?.epa.breakdown.auto_points.mean.toFixed(2)}</td>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    const firstRanking = (await tba.req.getCompetitonRanking(eventKey)).rankings;
    const tbaOPR = await tba.req.getCompetitonOPRS(eventKey);
    const statbotics = await Promise.all(
        firstRanking.map(async ({team_key}, index) => {
            return await Statbotics.getTeamEvent(eventKey, team_key.split("frc")[1]);
        })
    )

    return {
      props: {comp: JSON.parse(JSON.stringify(eventInformation)), firstRanking: firstRanking, oprRanking: tbaOPR, statbotics: statbotics},
    };
  };